...
BOOL TakingOutForm_OnInitDialog(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	...
	//1.5 단어카드철 프레임 윈도우에서 트리뷰컨트롤의 모든 항목들을 없앤다.
	SendMessage(GetDlgItem(wordCardBinderForm, IDC_TREE_ALPHABETINDEX), TVM_DELETEITEM, 0,
		(LPARAM)TVI_ROOT);
	//1.6 단어카드철 프레임 윈도우의 트리뷰컨트롤에서 "Alphabets"항목을 삽입한다.
	...
	//1.7 단어카드철 프레임 윈도우의 단어카드철에서 단어카드를 꺼낸 후에 단어카드가 남아 있으면
	//다시 단어카드철의 현재카드주소로 바꿔줌.
	currentCard = wordCardBinder->current;
	if (currentCard != NULL)
	{
		//1.7.1 단어카드철 프레임 윈도우에 현재 단어카드를 출력한다.
		...
		//1.7.2 단어카드철프레임 윈도우의 색인철에서 색인리스트를 만든다.
		IndexBinder_MakeList(indexBinder, &indexes, &count);
		//1.7.3 단어카드철 프레임윈도우에서 색인개수만큼 반복한다.
		i = 0;
		while (i < count)
		{
			//1.7.3.1 단어카드철 프레임윈도우의 트리뷰컨트롤에 알파벳항목을 삽입한다.
			...
			hAlphabet = (HTREEITEM)SendMessage(GetDlgItem(wordCardBinderForm, IDC_TREE_ALPHABETINDEX),
				TVM_INSERTITEM, 0, (LPARAM)&insertStruct);
			//1.7.3.2 단어카드철 프레임윈도우에서 색인에 저장된 단어카드 개수만큼 반복한다.
			j = 0;
			while (j < indexes[i].length)
			{
				//1.7.3.2.1 단어카드철 프레임윈도우에서 색인에 저장된 단어카드를 알아낸다.
				wordCard = Index_GetAt(indexes + i, j);
				//1.7.3.2.2 단어카드철 프레임윈도우에서 알아낸 단어카드의 철자와 다른 단어카드 사이에 철자중복이 있는지 확인한다.
				...
				//1.7.3.2.3 단어카드철 프레임윈도우에서 철자중복이 없으면
				if (isDuplicatedSpellings == FALSE)
				{
					//1.7.3.2.3.1 단어카드철 프레임윈도우에서 트리뷰컨트롤의 알파벳항목 아래에 철자를 삽입한다.
					...
					//1.7.3.2.3.2 단어카드철 프레임윈도우에서 색인에 저장된 단어카드들 중에서 중복된 철자의 단어카드들을 찾는다.
					Index_FindBySpellings(indexes + i, wordCard->word.spellings, &arrayIndexes,
						&arrayCount);
					//1.7.3.2.3.3 단어카드철 프레임윈도우에서 찾은 개수만큼 반복한다.
					m = 0;
					while (m < arrayCount)
					{
						//1.7.3.2.3.3.1 단어카드철 프레임윈도우에서 중복된 철자의 단어카드 중에서 중복된 품사가 있는지 확인한다.
						...
						//1.7.3.2.3.3.2 단어카드철 프레임윈도우에서 품사중복이 없으면
						if (isDuplicatedPartOfSpeech == FALSE)
						{
							//1.7.3.2.3.3.2.1 단어카드철 프레임윈도우에서 트리뷰 컨트롤에 철자항목 아래에 품사를 삽입한다.
							...
							hPartOfSpeech = (HTREEITEM)SendMessage(GetDlgItem(wordCardBinderForm,
								IDC_TREE_ALPHABETINDEX), TVM_INSERTITEM, 0, (LPARAM)&insertStruct);
							//1.7.3.2.3.3.2.2 단어카드철 프레임윈도우에서 트리뷰 컨트롤의 품사항목 아래에 의미를 삽입한다.
						...
						//1.7.3.2.3.3.3 단어카드철 프레임윈도우에서 품사가 중복되면
						else
						{
							//1.7.3.2.3.3.3.1 단어카드철 프레임윈도우에서 같은 품사를 찾는다.
							hPartOfSpeech = (HTREEITEM)SendMessage(GetDlgItem(wordCardBinderForm,
							...
							//1.7.3.2.3.3.3.2 단어카드철 프레임윈도우에서 같은 품사 항목 아래에 의미항목을 삽입한다.
							insertStruct.hParent = hPartOfSpeech;
							insertStruct.item.pszText = wordCard->word.meanings;
							SendMessage(GetDlgItem(wordCardBinderForm, IDC_TREE_ALPHABETINDEX),
								TVM_INSERTITEM, 0, (LPARAM)&insertStruct);
		...
		//1.7.4 단어카드철 프레임윈도우의 색인철에서 찾는다.
		indexLink = IndexBinder_Find(indexBinder, currentCard->word.spellings[0]);
		//1.7.5 단어카드철 프레임윈도우의 트리뷰컨트롤에서 찾은 색인의 알파벳 항목과 동일한 알파벳 항목을 찾는다.
		hAlphabets = (HTREEITEM)SendMessage(GetDlgItem(wordCardBinderForm, IDC_TREE_ALPHABETINDEX),
		...
		//1.7.6 단어카드철 프레임윈도우에서 트리뷰 컨트롤의 알파벳 항목에서 현재 단어카드의 철자를 찾는다.
		...
		//1.7.7 단어카드철 프레임윈도우에서 트리뷰컨트롤의 철자항목에서 현재 단어카드의 품사를 찾는다.
		...
		//1.7.8 단어카드철 프레임윈도우에서 트리뷰컨트롤에서 현재 출력된 단어카드의 의미와 동일한 의미를 찾는다.
		...
		//1.7.9 단어카드철 프레임윈도우의 트리뷰컨트롤에서 "Alphabet"항목을 펼친다.
		SendMessage(GetDlgItem(wordCardBinderForm, IDC_TREE_ALPHABETINDEX), TVM_EXPAND, (WPARAM)TVE_EXPAND,
			(LPARAM)hAlphabets);
		//1.7.10 단어카드철 프레임윈도우의 트리뷰컨트롤에서 현재 단어카드가 속하는 알파벳 항목을 펼친다.
		SendMessage(GetDlgItem(wordCardBinderForm, IDC_TREE_ALPHABETINDEX), TVM_EXPAND, (WPARAM)TVE_EXPAND,
			(LPARAM)hAlphabet);
		//1.7.11 단어카드철 프레임윈도우의 트리뷰컨트롤에서 현재 단어카드가 속하는 철자항목을 펼친다.
		SendMessage(GetDlgItem(wordCardBinderForm, IDC_TREE_ALPHABETINDEX), TVM_EXPAND, (WPARAM)TVE_EXPAND,
			(LPARAM)hSpellings);
		//1.7.12 단어카드철 프레임윈도우의 트리뷰컨트롤에서 현재 단어카드가 속하는 품사항목을 펼친다.
		SendMessage(GetDlgItem(wordCardBinderForm, IDC_TREE_ALPHABETINDEX), TVM_EXPAND, (WPARAM)TVE_EXPAND,
			(LPARAM)hPartOfSpeech);
		//1.7.13 단어카드철 프레임윈도우의 트리뷰컨트롤에서 현재 단어카드가 속하는 의미항목을 선택한다.
	...
	//1.8 단어카드철 프레임 윈도우의 단어카드철에서 단어카드를 꺼낸 후에 남아있는 단어카드가 없으면
	else
	{
		//1.8.1 단어카드철 프레임윈도우에 공란을 출력한다.
...
BOOL TakingOutForm_OnClose(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	...
	if (messageBoxButton == IDYES)
	{
		...
		SendMessage(GetDlgItem(wordCardBinderForm, IDC_STATIC_EDIT_EXAMPLE), WM_SETTEXT, 0,
			(LPARAM)currentCard->word.example);
		//2.2.6 단어카드철 프레임 윈도우의 색인철에서 색인리스트를 만든다.
		IndexBinder_MakeList(indexBinder, &indexes, &count);
		//2.2.7 단어카드철 프레임 윈도우의 트리뷰컨트롤에 있는 모든 항목들을 지운다.
		SendMessage(GetDlgItem(wordCardBinderForm, IDC_TREE_ALPHABETINDEX), TVM_DELETEITEM, 0,
			(LPARAM)TVI_ROOT);
		//2.2.8 단어카드철 프레임 윈도우의 트리뷰컨트롤에서 "Alphabets"항목을 삽입한다.
		insertStruct.hParent = TVI_ROOT;
		insertStruct.item.mask = TVIF_TEXT;
		insertStruct.item.pszText = "Alphabets";
		hAlphabets = (HTREEITEM)SendMessage(GetDlgItem(wordCardBinderForm, IDC_TREE_ALPHABETINDEX),
			TVM_INSERTITEM, 0, (LPARAM)&insertStruct);
		//2.2.9 단어카드철 프레임윈도우에서 색인개수만큼 반복한다.
		i = 0;
		while (i < count)
		{
			//2.2.9.1 단어카드철 프레임윈도우의 트리뷰컨트롤에 알파벳항목을 삽입한다.
			insertStruct.hParent = hAlphabets;
			indexesAlphabet[0] = indexes[i].alphabet;
			indexesAlphabet[1] = '\0';
			insertStruct.item.pszText = indexesAlphabet;
			hAlphabet = (HTREEITEM)SendMessage(GetDlgItem(wordCardBinderForm, IDC_TREE_ALPHABETINDEX),
				TVM_INSERTITEM, 0, (LPARAM)&insertStruct);
			//2.2.9.2 단어카드철 프레임윈도우에서 색인에 저장된 단어카드 개수만큼 반복한다.
			j = 0;
			while (j < indexes[i].length)
			{
				//2.2.9.2.1 단어카드철 프레임윈도우에서 색인에 저장된 단어카드를 알아낸다.
				wordCard = Index_GetAt(indexes + i, j);
				//2.2.9.2.2 단어카드철 프레임윈도우에서 알아낸 단어카드의 철자와 다른 단어카드 사이에 철자중복이 있는지 확인한다.
				isDuplicatedSpellings = FALSE;
				k = 0;
				while (k < j && isDuplicatedSpellings == FALSE)
				{
				...
				//2.2.9.2.3 단어카드철 프레임윈도우에서 철자중복이 없으면
				if (isDuplicatedSpellings == FALSE)
				{
					//2.2.9.2.3.1 단어카드철 프레임윈도우에서 트리뷰컨트롤의 알파벳항목 아래에 철자를 삽입한다.
					insertStruct.hParent = hAlphabet;
					insertStruct.item.pszText = wordCard->word.spellings;
					hSpellings = (HTREEITEM)SendMessage(GetDlgItem(wordCardBinderForm,
						IDC_TREE_ALPHABETINDEX), TVM_INSERTITEM, 0, (LPARAM)&insertStruct);
					//2.2.9.2.3.2 단어카드철 프레임윈도우에서 색인에 저장된 단어카드들 중에서 중복된 철자의 단어카드들을 찾는다.
					Index_FindBySpellings(indexes + i, wordCard->word.spellings, &arrayIndexes,
						&arrayCount);
					//2.2.9.2.3.3 단어카드철 프레임윈도우에서 찾은 개수만큼 반복한다.
					m = 0;
					while (m < arrayCount)
					{
						//2.2.9.2.3.3.1 단어카드철 프레임윈도우에서 중복된 철자의 단어카드 중에서 중복된 품사가 있는지 확인한다.
						wordCard = Index_GetAt(indexes + i, arrayIndexes[m]);
						isDuplicatedPartOfSpeech = FALSE;
						n = 0;
						while (n < m && isDuplicatedPartOfSpeech == FALSE)
						...
						//2.2.9.2.3.3.2 단어카드철 프레임윈도우에서 품사중복이 없으면
						if (isDuplicatedPartOfSpeech == FALSE)
						{
							//2.2.9.2.3.3.2.1 단어카드철 프레임윈도우에서 트리뷰 컨트롤에 철자항목 아래에 품사를 삽입한다.
							insertStruct.hParent = hSpellings;
							insertStruct.item.pszText = wordCard->word.partOfSpeech;
							hPartOfSpeech = (HTREEITEM)SendMessage(GetDlgItem(wordCardBinderForm,
								IDC_TREE_ALPHABETINDEX), TVM_INSERTITEM, 0, (LPARAM)&insertStruct);
							//2.2.9.2.3.3.2.2 단어카드철 프레임윈도우에서 트리뷰 컨트롤의 품사항목 아래에 의미를 삽입한다.
							insertStruct.hParent = hPartOfSpeech;
							insertStruct.item.pszText = wordCard->word.meanings;
							SendMessage(GetDlgItem(wordCardBinderForm, IDC_TREE_ALPHABETINDEX),
								TVM_INSERTITEM, 0, (LPARAM)&insertStruct);
						...
						//2.2.9.2.3.3.3 단어카드철 프레임윈도우에서 품사가 중복되면
						else
						{
							//2.2.9.2.3.3.3.1 단어카드철 프레임윈도우에서 같은 품사를 찾는다.
							hPartOfSpeech = (HTREEITEM)SendMessage(GetDlgItem(wordCardBinderForm,
								IDC_TREE_ALPHABETINDEX), TVM_GETNEXTITEM, (WPARAM)TVGN_CHILD,
								(LPARAM)hSpellings);
							item.mask = TVIF_TEXT;
							...
							//2.2.9.2.3.3.3.2 단어카드철 프레임윈도우에서 같은 품사 항목 아래에 의미항목을 삽입한다.
							insertStruct.hParent = hPartOfSpeech;
							insertStruct.item.pszText = wordCard->word.meanings;
							SendMessage(GetDlgItem(wordCardBinderForm, IDC_TREE_ALPHABETINDEX),
								TVM_INSERTITEM, 0, (LPARAM)&insertStruct);
		...
		//2.2.10 단어카드철 프레임윈도우의 색인철에서 찾는다.
		indexLink = IndexBinder_Find(indexBinder, currentCard->word.spellings[0]);
		//2.2.11 단어카드철 프레임윈도우의 트리뷰컨트롤에서 찾은 색인의 알파벳 항목과 동일한 알파벳 항목을 찾는다.
		hAlphabets = (HTREEITEM)SendMessage(GetDlgItem(wordCardBinderForm, IDC_TREE_ALPHABETINDEX),
			TVM_GETNEXTITEM, (WPARAM)TVGN_ROOT, (LPARAM)NULL);
		hAlphabet = (HTREEITEM)SendMessage(GetDlgItem(wordCardBinderForm, IDC_TREE_ALPHABETINDEX),
			TVM_GETNEXTITEM, (WPARAM)TVGN_CHILD, (LPARAM)hAlphabets);
		...
		//2.2.12 단어카드철 프레임윈도우에서 트리뷰 컨트롤의 알파벳 항목에서 현재 단어카드의 철자를 찾는다.
		hSpellings = (HTREEITEM)SendMessage(GetDlgItem(wordCardBinderForm, IDC_TREE_ALPHABETINDEX),
			TVM_GETNEXTITEM, (WPARAM)TVGN_CHILD, (LPARAM)hAlphabet);
		item.mask = TVIF_TEXT;
		item.pszText = spellings;
		...
		//2.2.13 단어카드철 프레임윈도우에서 트리뷰컨트롤의 철자항목에서 현재 단어카드의 품사를 찾는다.
		hPartOfSpeech = (HTREEITEM)SendMessage(GetDlgItem(wordCardBinderForm, IDC_TREE_ALPHABETINDEX),
			TVM_GETNEXTITEM, (WPARAM)TVGN_CHILD, (LPARAM)hSpellings);
		item.mask = TVIF_TEXT;
		item.pszText = partOfSpeech;
		...
		//2.2.14 단어카드철 프레임윈도우에서 트리뷰컨트롤에서 현재 출력된 단어카드의 의미와 동일한 의미를 찾는다.
		hMeanings = (HTREEITEM)SendMessage(GetDlgItem(wordCardBinderForm, IDC_TREE_ALPHABETINDEX),
			TVM_GETNEXTITEM, (WPARAM)TVGN_CHILD, (LPARAM)hPartOfSpeech);
		item.mask = TVIF_TEXT;
		item.pszText = meanings;
		...
		//2.2.15 단어카드철 프레임윈도우의 트리뷰컨트롤에서 "Alphabet"항목을 펼친다.
		SendMessage(GetDlgItem(wordCardBinderForm, IDC_TREE_ALPHABETINDEX), TVM_EXPAND, 
			(WPARAM)TVE_EXPAND, (LPARAM)hAlphabets);
		//2.2.16 단어카드철 프레임윈도우의 트리뷰컨트롤에서 현재 단어카드가 속하는 알파벳 항목을 펼친다.
		SendMessage(GetDlgItem(wordCardBinderForm, IDC_TREE_ALPHABETINDEX), TVM_EXPAND, 
			(WPARAM)TVE_EXPAND, (LPARAM)hAlphabet);
		//2.2.17 단어카드철 프레임윈도우의 트리뷰컨트롤에서 현재 단어카드가 속하는 철자항목을 펼친다.
		SendMessage(GetDlgItem(wordCardBinderForm, IDC_TREE_ALPHABETINDEX), TVM_EXPAND, 
			(WPARAM)TVE_EXPAND, (LPARAM)hSpellings);
		//2.2.18 단어카드철 프레임윈도우의 트리뷰컨트롤에서 현재 단어카드가 속하는 품사항목을 펼친다.
		SendMessage(GetDlgItem(wordCardBinderForm, IDC_TREE_ALPHABETINDEX), TVM_EXPAND, 
			(WPARAM)TVE_EXPAND, (LPARAM)hPartOfSpeech);
		//2.2.19 단어카드철 프레임윈도우의 트리뷰컨트롤에서 현재 단어카드가 속하는 의미항목을 선택한다.
