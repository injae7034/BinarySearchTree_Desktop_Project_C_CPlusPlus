...
//Index_FindBySpellings
//arrayIndexes 배열을 count와 함께 main으로 출력해야하므로 주소로 main에 있는 arrayIndex에 출력함. 자료형 주의하기!
void Index_FindBySpellings(Index* index, char(*spellings), Long*(*arrayIndexes), Long* count)
{
	//1. 철자를 입력받는다.
	//2. 같은 철자를 찾는다.
	Array_LinearSearchDuplicate(&index->wordCards, spellings, arrayIndexes, count, sizeof(WordCard*),
